...
# -𝞉Q/∂A
function J(u::Vector{Float64}, α::Float64)
    return α * u + 1 / (σ^2) * (-diagm(S) * U' * G + d * diagm(S)^2 * V' * A_vec(u))
end

# -∂²Q/∂A∂u
function H(u::Vector{Float64}, α::Float64)
    return α * Matrix(I(n)) + d / (σ^2) * diagm(S)^2 * V' * diagm(A_vec(u)) * V
end
