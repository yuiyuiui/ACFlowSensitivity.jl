...
function precompute(
	...
	# Compute Wₘₗ
    @einsum W₂[m, l] = σ²[k] * (U[k, n] * U[k, m]) * (S[m] * S[n] * V[l, n]) * (mesh_weight[l] * model[l])
	...
	@einsum hess[i, j] = mesh_weight[i] * mesh_weight[j] * (kernel[k, i] * kernel[k, j] * σ²[k])
...
function svd_to_real_od()
end

function calc_entropy_od()
end
