...
function chi2kink(ss::SingularSpace{T}, model::Vector{T}, w::Vector{T}, L::Int, α₁::T,
    ...
    u_opt, = newton(MaxEnt_J(α_opt, DSUadDivσ², KDw, model, V, G),
                    MaxEnt_H(α_opt, S²VadDwDivσ², model, V), u_guess)
    # recover the A
    return A(u_opt)
end
...
function solvediff(GFV::Vector{Complex{T}}, ctx::CtxData{T},
    ...
    ss = SingularSpace(GFV, ctx.mesh, ctx.iwn)
    reA = make_model(alg.model_type, ctx)
    ∂GDiv∂A = Matrix{Complex{T}}(undef, length(GFV), length(ctx.mesh))
    for i in 1:maxiter
        model = reA
        reA = chi2kink(ss, model, ctx.mesh_weights, L, α₁, σ)
        if i == maxiter
            ∂GDiv∂A = chi2kink_diff(ss, model, ctx.mesh_weights, L, α₁, σ)
        end
    end
    return ctx.mesh, reA, ∂GDiv∂A
end
