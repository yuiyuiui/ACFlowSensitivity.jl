...
function chi2kink(G::Vector{T}, K::Matrix{T}, n::Int, U::Matrix{T}, S::Vector{T},
    ...
    for i in 1:L
        α = α_vec[i]
        u_opt, _, _ = my_newton(u -> J(u, α), u -> H(u, α), u_guess)
        u_guess = copy(u_opt)
    ...
    _, _, c, _ = my_curve_fit(log10.(α_vec), log10.(χ²_vec), guess_fit, Newton())[1]
...
function my_chi2kink(iwn::Vector{ComplexF64},
...
function ADchi2kink(iwn::Vector{ComplexF64},
                    Gvalue::Vector{ComplexF64},
                    output_range::Vector{Float64})
...
function _ADchi2kink(iwn::Vector{ComplexF64},
    ...
    ∂χ²div∂A(u::Vector{Float64}) = Matrix(2 / (σ^2) * (-d * G' * K +
                                                       d^2 * A_vec(u)' * V * diagm(S .^ 2) * V'))
    ...
    dpdivdχ²_vec = -pinv(∂²loss_curveDiv∂p²(arg...)) *
                   ∂²loss_∂p∂y(arg...) *
...
function ADchi2kink_v2(iwn::Vector{ComplexF64},
                       Gvalue::Vector{ComplexF64},
                       output_range::Vector{Float64})
...
function _ADchi2kink_v2(iwn::Vector{ComplexF64},
                        Gvalue::Vector{ComplexF64},
                        output_range::Vector{Float64})
    output_number = length(output_range)
    N = length(Gvalue)
