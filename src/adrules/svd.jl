...
function svd_back(U::AbstractArray, S::AbstractArray{T}, V, dU, dS, dV;
                  η::Real=1e-40) where {T}
    ...
    if !(dU isa Nothing)
        UdU = U'*dU
        J = F .* (UdU)
        res += (J+J')*LinearAlgebra.Diagonal(S) +
               LinearAlgebra.Diagonal(1im*imag(LinearAlgebra.diag(UdU)) .* Sinv)
    end
    if !(dV isa Nothing)
        VdV = V'*dV
        K = F .* (VdV)
...
Zygote.@adjoint function svd(A)
    ...
    function pullback(dy)
        return (svd_back(U, S, V, dy[1], dy[2], dy[3]),)
    end
