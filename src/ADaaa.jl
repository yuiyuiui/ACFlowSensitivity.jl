    ...
    for j ∈ 1:m
        ...
        for i ∈ 1:(n-m)
            ...
            for k ∈ 1:n
                if k == f.Index0[1][i]
                    jac[(j-1)*(n-m)+i, k] =
                        1 / (f.iwn[f.Index0[1][i]] - f.iwn[f.Index0[2][j]])
...
function aaa_cont_FiniteDifference_Direct(
    wn::Vector{Float64},
    Giwn::Vector{ComplexF64};
    int_low::Float64 = -8.0,
    int_up::Float64 = 8.0,
