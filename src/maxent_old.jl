...
function PreComput(GFV::Vector{Complex{T}}, ctx::CtxData{T},
    ...
    σ = T(ctx.σ)
    w = ctx.mesh.weight
    ss = SingularSpace(GFV, ctx.wn, ctx.mesh.mesh)
...
function solve_old(GFV::Vector{Complex{T}}, ctx::CtxData{T},
                   alg::MaxEnt) where {T<:Real}
    ...
    elseif ctx.spt isa Delta
        p = ctx.mesh.mesh[find_peaks(ctx.mesh.mesh, reA, ctx.fp_mp; wind=ctx.fp_ww)]
...
function G2χ²vec(pc::PreComput{T}) where {T<:Real}
    ...
    for i in 1:length(αvec)
        #@show i
        α = αvec[i]
        _J = MaxEnt_J(α, pc.DSUadDivσ², pc.KDw, pc.model, V, G)
        _H = MaxEnt_H(α, pc.S²VadDwDivσ², pc.model, V)
        u_opt, call, _ = newton(x -> (_J(x), _H(x)), u_guess)
        u_guess = copy(u_opt)
...
function chi2kink(pc::PreComput{T}) where {T<:Real}
    ...
    _J = MaxEnt_J(αopt, pc.DSUadDivσ², pc.KDw, pc.model, pc.ss.V, pc.ss.G)
    _H = MaxEnt_H(αopt, pc.S²VadDwDivσ², pc.model, pc.ss.V)
    u_opt, = newton(x->(_J(x), _H(x)), u_guess)
    # recover the A
    return _A(u_opt)
end
...
function solvediff_old(GFV::Vector{Complex{T}}, ctx::CtxData{T},
                       alg::MaxEnt) where {T<:Real}
    if ctx.spt isa Cont
        pc = PreComput(GFV, ctx, alg)
        return chi2kink_diff(pc, alg)
    elseif ctx.spt isa Delta
...
function _∂χ²vecDiv∂G(pc::PreComput{T}, alg::MaxEnt) where {T<:Real}
    G, K, n, U, _, V = pc.ss
    αvec = pc.αvec
    σ = pc.σ
    model = pc.model
    ...
    for i in 1:nalph
        ...
        _Jᵢ = MaxEnt_J(α, pc.DSUadDivσ², pc.KDw, model, V, G)
        uᵢ, = newton(x -> (_Jᵢ(x), _Hᵢ(x)), u_guess)
        u_guess = copy(uᵢ)
        u_opt_vec[i] = copy(uᵢ)
        χ²vec[i] = _χ²(uᵢ)
...
function chi2kink_diff(pc::PreComput{T}, alg::MaxEnt) where {T<:Real}
    ss = pc.ss
    model = pc.model
    G, _, _, U, _, V = pc.ss
    ∂χ²vecDiv∂G, u_opt_vec, χ²vec, idx = _∂χ²vecDiv∂G(pc, alg)
    ...
    _J = MaxEnt_J(αopt, pc.DSUadDivσ², pc.KDw, model, V, G)
    u_opt, = newton(x -> (_J(x), _Hopt(x)), u_guess)
    ...
    return _A(u_opt), ∂AoptDiv∂G[:, 1:N] + im * ∂AoptDiv∂G[:, (N + 1):(2 * N)], alg.test
end
