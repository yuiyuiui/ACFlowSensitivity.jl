...
p_delta = Vector{Vector{T}}(undef, alg_num)
γ_delta = Vector{Vector{T}}(undef, alg_num)
...
# SAC
ctx_delta_sac = dfcfg(Float64, Delta(); npole=2, fp_ww=0.1, fp_mp=2.0, mb=4, ml=2000)[2]
...
for j in 1:alg_num
    record_delta("Delta_$(method_name_vec[j])", sst_delta[j],
...
for j in 1:alg_num
    record_cont("Mixed_$(method_name_vec[j])", sst_mixed[j],
